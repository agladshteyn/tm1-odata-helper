...
const tm1Client = require("./tm1client");
const tiprocess = require("./tiprocess");
...
function activate(context) {
    ...
    let markdown = new markdown_1.Markdown();
    let buildConnection = function (connName, callback) {
        // Prompt for the OData URL.
        vscode.window.showInputBox({ prompt: "Enter OData URL (example: http://localhost:1234)", placeHolder: "OData URL", ignoreFocusOut: true }).then((url) => {
            let quickPickOpts = {
            ...
            vscode.window.showQuickPick(connTypes, quickPickOpts).then((connType) => {
                if (connTypes.indexOf(connType) === -1) {
                    vscode.window.showErrorMessage(`Invalid connection type: ${connType}`);
                    return;
                }
                // Prompt for username.
                vscode.window.showInputBox({ prompt: "Enter TM1 username", placeHolder: "TM1 username", ignoreFocusOut: true }).then((username) => {
                    if (!username || username === "") {
                        vscode.window.showErrorMessage(`Username is required`);
                        return;
                    }
                    // Prompt for password.
                    vscode.window.showInputBox({ prompt: "Enter TM1 password", placeHolder: "TM1 password", ignoreFocusOut: true }).then((password) => {
                        ...
                        if (connType === "CAM") { // If connecting using CAM, prompt for CAM namespace also
                            vscode.window.showInputBox({ prompt: "Enter CAM namespace", placeHolder: "CAM namespace (example: LDAP)", ignoreFocusOut: true }).then((namespace) => {
    ...
    let cmdNewTM1Connection = vscode.commands.registerCommand('extension.NewConnection', () => {
        ...
        vscode.window.showInputBox({ prompt: "Enter Connection Name", placeHolder: "Connection Name (example: Prod)", ignoreFocusOut: true }).then((connName) => {
    ...
    let cmdNewTIDefn = vscode.commands.registerCommand('extension.NewTIProcessTemplate', () => {
        ...
        vscode.window.showInputBox({ prompt: "Enter Process Name", placeHolder: "Process Name", ignoreFocusOut: true }).then((name) => {
            if (!name || name === "") {
                vscode.window.showErrorMessage(`Process name is required`);
                return;
            }
            // Prompt for the number of parameters.
            vscode.window.showInputBox({ prompt: "Number of Process Parameters", placeHolder: "Number of Process Parameters", ignoreFocusOut: true }).then((paramCountStr) => {
                let paramCount = Number.parseInt(paramCountStr);
                if (isNaN(paramCount))
                    paramCount = 0;
                // Prompt for the number of variables.
                vscode.window.showInputBox({ prompt: "Number of Process Variables", placeHolder: "Number of Process Variables", ignoreFocusOut: true }).then((varCountStr) => {
                    let varCount = Number.parseInt(varCountStr);
                    if (isNaN(varCount))
                        varCount = 0;
                    let dsTypes = Object.keys(process_1.TIProcessDataSourceType);
                    let quickPickOpts = {
                        placeHolder: "Select data source type",
                        ignoreFocusOut: true
                    };
                    vscode.window.showQuickPick(dsTypes, quickPickOpts).then((dsType) => {
    ...
    let cmdCreateTIProcess = vscode.commands.registerCommand('extension.CreateTIProcess', () => {
        ...
        let procDefnJson;
        try {
            procDefn = markdown.deserialize(text, process_1.TIProcessDefn.prototype);
        ...
        if (connections.length > 0) {
            vscode.window.showQuickPick(connections, { placeHolder: "Select the Target TM1 Connection", ignoreFocusOut: true }).then((connName) => {
                let connection = null;
                if (connName && connName !== "")
